...
DBPATH = "/Data/home/ken/TextMateProjects/TransferScans/db/development.sqlite3"
...
class YamlProcessor:
	...
	def create_yaml_files(self, options):
		...
		visit_id = list(CURSOR)[0][0]
		# Find the details for all of the scans associated with that visit.
		CURSOR.execute("SELECT series_description, path, glob FROM image_datasets WHERE id=?", (visit_id,))
		# Go through the scans and create the yaml files.
		for series_description, path, glob in list(CURSOR):
...
#		return result
	...
	def make_yaml_hash(self, series_description, anat_type, glob, prefix, output_dir, output_file):
		if anat_type == "anat":
			result = dict(
				input_dir = path,
	...
	####################################################################
	def process_scan_directory(self):
	...
	# Create the yaml files for a visit directory.
	def process_scan_directory2(self, options):
		# Find the id of the given visit
		CURSOR.execute("SELECT id FROM visits WHERE path=?", (self.visit_dir,))
		assert len(list(CURSOR)) == 1
		visit_id = list(CURSOR)[0][0]
		# Find the details for all of the scans associated with that visit.
		CURSOR.execute("SELECT path FROM image_datasets WHERE id=?", (visit_id,))
		for (path,), in list(CURSOR):
			parent_path, scan_dir = os.path.split(path)
			#yaml_input = os.path.join(parent_path, scan_dir + ".yaml")
			print "Will read yaml file to ", os.path.join(parent_path, scan_dir + ".yaml")
	...
	# This function is no longer used and should probably be removed.
	def execute2(self, cmd, error_message):
...
def process_directory(visit_dir):
	...
	yp.process_scan_directory()

if __name__ == "__main__":
	if sys.argv[0].endswith("process"):
